...
- **Content API (audio/hadith) is not QF User API.** Content API access uses backend client credentials with `scope=content` through `supabase/functions/qf-content`; never put `QF_CONTENT_CLIENT_SECRET` or `QF_CLIENT_SECRET` in Expo, web, or any client bundle.
- **Required Content API deployment:** set Supabase secrets `QF_CONTENT_CLIENT_ID`, `QF_CONTENT_CLIENT_SECRET`, `QF_CONTENT_ENV=production|prelive`, and `QF_DEFAULT_RECITATION_ID=6` (Mahmoud Khalil Al-Husary, Murattal), then deploy `supabase functions deploy qf-content`. Missing secrets or an undeployed function will surface in the UI as unavailable audio/hadith.
