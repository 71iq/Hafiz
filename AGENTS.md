...
- **Required Content API deployment:** set Supabase secrets `QF_CONTENT_CLIENT_ID`, `QF_CONTENT_CLIENT_SECRET`, `QF_CONTENT_ENV=production|prelive`, and `QF_DEFAULT_RECITATION_ID=7`, then deploy `supabase functions deploy qf-content`. Missing secrets or an undeployed function will surface in the UI as unavailable audio/hadith.
