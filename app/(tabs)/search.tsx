...
// ─── Highlight helper ───
// Maps a match found in diacritics-stripped text back to original text positions
function buildHighlightSegments(
  ...
  // Map stripped char index → original char index
  let strippedPos = 0;
  let origStart = -1;
  let origEnd = -1;
  ...
  // Include trailing diacritics in the highlight
...
// For root search: highlight a specific word in the ayah by word_text
function highlightWord(
  original: string,
  wordText: string,
): { text: string; highlight: boolean }[] {
  // Try to find word_text (with diacritics) directly
  ...
  // Fallback: try stripped matching
...
export default function SearchScreen() {
    ...
    async (term: string, searchMode: "text" | "root") => {
      ...
      try {
        if (searchMode === "text") {
          ...
          // Root search — exact match on root column
  ...
  // Mode switch triggers re-search
  ...
  return (
    <SafeAreaView className="flex-1 bg-surface dark:bg-surface-dark" edges={["top"]}>
      ...
      ) : hasSearched && mode === "text" && textResults.length === 0 ? (
        // ─── No results ───
      ...
      ) : hasSearched && mode === "root" && rootResults.length === 0 ? (
        <View className="flex-1 items-center justify-center px-6">
          <Search size={40} color={isDark ? "#404040" : "#DFD9D1"} />
          <Text
            className="text-warm-400 dark:text-neutral-500 mt-4"
                ...
                {group.results.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Ayah text with highlighted match */}
                      <Text
                        style={{
