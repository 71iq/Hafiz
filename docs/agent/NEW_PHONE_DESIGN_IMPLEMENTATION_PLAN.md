...
Status: reference-only historical roadmap. When this file overlaps with live UI rules or verification gates, `docs/agent/WEB_UI_CONTRACT.md` is authoritative.

Historical source folder: `new phone design/` (archival and not guaranteed to exist in the current worktree).

This document is reference guidance only. It intentionally does not implement the new design and it does not replace the live route, verification, or responsive contract. Use it as a historical phase checklist when the archival mockup references are available.

## Design Inventory

Active repo references:

- `docs/product/DESIGN_SYSTEM.md`: live visual north star, tokens, typography, spacing, motion, and component rules.
- `docs/product/DESIGN_PHILOSOPHY.md`: companion narrative for the design direction.
- `docs/product/HAFIZ_SPEC.md`: product and architecture specification.
- `docs/agent/WEB_UI_CONTRACT.md`: authoritative UI rules and verification gates.
- `docs/agent/PHONE_PLAN_AGENT_LOG.md`: continuity log for redesign and stabilization work.

Historical mockup references, when available locally:
...
- A short implementation decision note is appended to this file or `docs/product/HAFIZ_SPEC.md`.
...
- Audit existing `tailwind.config.js`, `global.css`, `docs/product/DESIGN_SYSTEM.md`, and `components/ui/*`.
...
- Document capture preconditions and known limits in `docs/agent/PHONE_PLAN_AGENT_LOG.md`.
