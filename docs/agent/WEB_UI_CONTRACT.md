...
- The canonical route shell for non-reader app screens is `components/ui/ScreenContent.tsx`: use `Screen` for safe-area/background ownership and `ScreenScrollView` for scroll insets, width rails, gutters, and scroll defaults.
- Main non-reader tab screens should use the shared `TAB_SCREEN_BOTTOM_INSET` unless a documented compact surface needs `COMPACT_SCREEN_BOTTOM_INSET`.
...
- Current migrated consumers include search, confirmation dialogs, translation picker, deck sheets, bookmarks, go-to navigator, ayah detail, word detail, reflections sheets, private notes, recitation range, reading settings, profile sheets, and reflection-feed filters.
- Legacy overlay systems such as `components/ui/Sheet.tsx`, `MobileBottomSheet`, specialized dropdown/page-navigation popovers, and `SelectionActionBar` remain in place until later migration passes.
