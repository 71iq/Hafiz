...
## Scope
This note treats Quran.com as a read-only UX and engineering reference. It is based on the local `quran.com-frontend-next/` checkout currently present in the Hafiz worktree:
...
- The local inventory shows a deliberate separation between generic UI pieces and feature modules:
  - `Button`
  - `Card`
  - `ContentModal`
  - `ConfirmationModal`
...
- The local Playwright config defines both desktop and mobile browser projects.
...
- The local `quran.com-frontend-next/` checkout is reference-only.
