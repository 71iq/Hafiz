...
| Tab press scale                     | `withSpring(0.98, { damping: 15, stiffness: 400 })`. Older `0.9` guidance is obsolete. |
...
- **Bottom bar (mobile, < 768px):** absolute at bottom, rounded top corners, glassmorphic background (`surface` at 80% opacity, `backdrop-blur: 14px` on web, stronger opacity on native). Active item gets a `primary-soft` (`#1B4D4F`) pill with `gold` (`#FDDC91`) icon + label. Press scale follows the live `0.98` interaction.
- **Sidebar (desktop, ≥ 768px):** floating `248px` panel offset from the viewport edge, not a fixed `220px` sidebar. The panel header reads "Hafiz" / "The Digital Sanctuary" in Noto Serif Bold + Manrope. Same active pill treatment.
...
The app has 5 visible primary tabs, accessed via responsive bottom bar (mobile) or floating sidebar panel (desktop):
...
**Hidden or auxiliary routes in the shipped graph:**
- `app/(tabs)/search.tsx` — hidden redirect to Home; real search lives in `components/SearchCommand.tsx`
- `app/(tabs)/flashcards.tsx` — hidden redirect to Home
- `app/flashcards/session.tsx` — full flashcard review session outside tabs
- `app/flashcards/vocab.tsx` — vocab review route outside tabs
- `app/onboarding.tsx` — 3-screen onboarding flow
- `app/auth/login.tsx`, `app/auth/signup.tsx`, `app/auth/forgot-password.tsx`, `app/auth/reset-password.tsx` — auth routes
- `app/profile/[userId].tsx` — public leaderboard profile
- `app/qa-ready.tsx` — deterministic QA readiness route
- `app/open.tsx` — deep-link bridge
- `app/+not-found.tsx` — catch-all fallback route
...
- `docs/product/DESIGN_PHILOSOPHY.md` — companion narrative document.
- `docs/product/HAFIZ_SPEC.md` — product and architecture specification.
- `docs/agent/CODEBASE_MAP.md` — current route, provider, primitive, and data-surface map.
- `docs/agent/UI_AUDIT.md` — current stabilization findings and known drift.
- `docs/agent/WEB_UI_CONTRACT.md` — implementation and verification contract for the web UI pass.
- `docs/agent/QURAN_FRONTEND_REFERENCE_NOTES.md` — read-only Quran.com reference notes.
- `docs/agent/PHONE_PLAN_AGENT_LOG.md` — continuity log for redesign and stabilization work.
- `docs/design-mockups/` — currently present but empty in this checkout; do not assume ready-to-use mockups live there.
- `stitch_review_session/` — referenced in earlier docs, but not present in this checkout.
- `design-references/` — referenced in earlier docs, but not present in this checkout.
