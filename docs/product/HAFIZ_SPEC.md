...
- Quran Foundation Content APIs are optional online enrichment. Audio and hadith-by-ayah go through the Supabase Edge Function `qf-content` using backend client credentials with `scope=content`; `QF_CONTENT_CLIENT_SECRET` and `QF_CLIENT_SECRET` must never be shipped to Expo/native/web clients.
- Content API availability requires deployed Supabase secrets: `QF_CONTENT_CLIENT_ID`, `QF_CONTENT_CLIENT_SECRET`, `QF_CONTENT_ENV=production|prelive`, and `QF_DEFAULT_RECITATION_ID=6` (Mahmoud Khalil Al-Husary, Murattal). If these are absent or the Edge Function is not deployed, UI must show bilingual unavailable states while local SQLite Quran reading continues.
