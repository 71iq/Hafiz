...
- Content API availability requires deployed Supabase secrets: `QF_CONTENT_CLIENT_ID`, `QF_CONTENT_CLIENT_SECRET`, `QF_CONTENT_ENV=production|prelive`, and `QF_DEFAULT_RECITATION_ID=7`. If these are absent or the Edge Function is not deployed, UI must show bilingual unavailable states while local SQLite Quran reading continues.
